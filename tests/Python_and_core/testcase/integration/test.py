...
class IntegrationTest(base.BaseTestCase):
    ...
    def test_args_must_be_list(self):
        with self.assertRaisesRegexp(ValueError, "args must be a list"):
            sandbox.run(max_cpu_time=1000, max_real_time=2000,
    ...
    def test_cpu_time(self):
        ...
        result = sandbox.run(**config)
        self.assertEqual(result["result"], sandbox.RESULT_CPU_TIME_LIMIT_EXCEEDED)
        self.assertEqual(result["signal"], signal.SIGKILL)
    ...
    def test_memory1(self):
        ...
        self.assertEqual(result["result"], sandbox.RESULT_MEMORY_LIMIT_EXCEEDED)
    ...
    def test_gcc_random(self):
        config = self.base_config
        config["max_memory"] = sandbox.UNLIMITED
